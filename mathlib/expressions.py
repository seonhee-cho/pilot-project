"""
magic function (__add / __sub / __mul / __truediv / __pow 등)
"""
...
class BinOpExpression(Expression):
    ...
    def differentiate(self, var: str = "", direction: list[Decimal]=None) -> 'Expression':
        ...
        """
        if var:
            return self.derivative(var).canonicalize()
        elif direction:
            return [self.derivative(var).canonicalize() for var in direction]
        else: # gradient 반환
            return self.gradient().canonicalize()
...
class SingleVarFunction(Expression):
    ...
    def derivative(self, var: str) -> 'Expression':
        ...
        if isinstance(derivative_expr, SingleVarFunction) or isinstance(derivative_expr, MultiVarFunction):
            # 체인 룰 적용
            outer_derivative = SingleVarFunction(self.func, self.expr)
            return BinOpExpression(outer_derivative, '*', derivative_expr)
    ...
    def _repr_tree(self, level=0):
        return f"{'  ' * level}SingleVarFunction({self.func})\n{self.expr._repr_tree(level + 2)}".replace("((", "(").replace("))", ")")
...
class MultiVarFunction(Expression):
    ...
    def __str__(self):
        return f"{self.func}({', '.join(str(arg) for arg in self.args)})".replace("((", "(").replace("))", ")")
